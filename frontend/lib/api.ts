...
export const api = {
  ...
  registerDataset(
    ...
    return request<{ dataset_id: string; status: string }>(
  ...
  listDatasets(
    ...
    const params = new URLSearchParams({ organization_id: organizationId });
    if (projectId) params.set("project_id", projectId);
    return request<{ datasets: any[] }>(
      `/api/v1/datasets?${params}`,
      { token }
    );
  },
  ...
  createAnalysis(
    ...
    return request<{ job_id: string; status: string }>("/api/v1/analyses", {
      method: "POST",
      body: JSON.stringify(body),
      token,
    });
  ...
  getArtifactUrl(jobId: string, token: string) {
    ...
    }>(`/api/v1/analyses/${jobId}/artifact`, { token });
  },
};
