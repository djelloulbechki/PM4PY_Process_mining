...
export default function JobDetailPage({ params }: { params: Promise<{ jobId: string }> }) {
  ...
  return <div className="space-y-6">
    <div className="flex items-start justify-between gap-4">
      ...
      {job && <div className="flex items-center gap-2"><Badge tone={statusTone(job.status)}>{job.status}</Badge>{["queued","processing","retrying"].includes(job.status) && <Button size="sm" variant="danger" onClick={handleCancel}>Cancel</Button>}</div>}
    </div>
