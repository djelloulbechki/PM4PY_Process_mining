...
def get_optimization_suggestions(
    ...
    """
    Generate rule-based optimization suggestions from a completed analysis.
    Returns: suggestions, flow_diagram (React Flow), n8n_blueprint.
    """
    from app.services.optimization import generate_optimization_suggestions
    import json
    ...
    if job.data["status"] != "completed":
        raise HTTPException(
            status_code=409,
            detail="Optimization suggestions are only available for completed jobs.",
        )

    result = (
        sb.table("analysis_results")
        .select("metrics_summary,artifact_storage_path")
        .eq("job_id", job_id)
        .single()
        .execute()
    )
    ...
    process_data: dict = result.data.get("metrics_summary") or {}

    # Prefer full artifact when available
    path = result.data.get("artifact_storage_path")
    if path:
        try:
            artifact_bytes = worker_supabase.storage.from_(settings.artifacts_bucket).download(path)
            process_data = json.loads(artifact_bytes)
        except Exception:
            # Fall back to metrics_summary already loaded
            pass

    optimization = generate_optimization_suggestions(process_data)
    return optimization
